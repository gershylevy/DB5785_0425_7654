...
], columns=['segment_id', 'segment_name', 'description', 'min_balance_required'])

# Save segment template
segment_df.to_excel('excel_templates/segment_template.xlsx', index=False)

# ---------------------- Segment Assignment Template ----------------------
segment_assignment_df = pd.DataFrame(columns=[
    'assignment_id', 'customer_id', 'segment_id', 'assigned_date'
])

# Add 10 sample rows
for i in range(1, 11):
    customer_id = i  # One segment per customer for sample
    segment_id = random.randint(1, 10)
    assigned_date = fake.date_between(start_date='-5y', end_date='today').strftime('%Y-%m-%d')
    
    segment_assignment_df.loc[i-1] = [
        i,
        customer_id,
        segment_id,
        assigned_date
...
print("Excel templates have been created in the 'excel_templates' directory:")
print("1. customer_template.xlsx")
print("2. address_template.xlsx")
print("3. contact_template.xlsx")
print("4. document_template.xlsx")
print("5. note_template.xlsx")
print("6. segment_template.xlsx")
print("7. segment_assignment_template.xlsx")
